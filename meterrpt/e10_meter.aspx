...
<div id="modal_edit_prov" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-nm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">แก้ไขจังหวัด พื้นที่รับผิดชอบ</h4>
            </div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-success" id="btn_save_prov">บันทึกหมายเหตุ</button>
              <button type="button" class="btn btn-primary" id="btn_cf_save_prov" style="display: none;">ยืนยัน</button>
              <button type="button" class="btn btn-danger" data-dismiss="modal">ปิด</button>
            </div>
        </div>
    </div>
    ...
    $.ajax({
        ...
        success: function( data ) { 
          if (data.length > 0) {
            ...
            "    <th colspan='5' style='text-align:center;'>ข้อมูล LOCATION</th> " +
            "    <th colspan='3' style='text-align:center;'>พื้นที่รับผิดชอบ</th> " +
            ...
            $.each(data,function( i,item ) {
              var CONTRACT_ID = '<a title="แก้ไขข้อมูล LOCATION" href="#" role="button" onclick="modalEditMaster(\'' + item.CONTRACT_ID + '\')"><span class="glyphicon icon-feather-edit3"></span></a> ' + item.CONTRACT_ID + "";
              var LOCATION = '-';
              var PROVINCE = '<a title="แก้ไขพื้นที่รับผิดชอบ" href="#" role="button" onclick="modalEditProvince(\'' + item.CONTRACT_ID + '\',\'' + item.PROVINCE + '\')"><span class="glyphicon icon-feather-edit3"></span></a> ';
              ...
              if(checkNotEmpty(item.PROVINCE) == true) {
...
function loadPROV() {
    ...
    $.ajax({
        ...
        success: function(data) {
            $el.empty();
            $el.append($("<option></option>")
                .attr("value", "").text("กรุณาเลือกจังหวัด"));
...
function resetEditProvince() {
  $('#hide_CONTRACT_ID').val("");
  $('#SEL_PROVINCE').val("");
  $('#btn_cf_save_prov').hide();
  $('#btn_save_prov').show();
}

function modalEditProvince(CONTRACT_ID, PROVINCE) {
  resetEditProvince();

  $('#hide_CONTRACT_ID').val(CONTRACT_ID);
  
  if(checkNotEmpty(PROVINCE)) {
    $('#SEL_PROVINCE').val(PROVINCE);
  }

  $('#modal_edit_prov').modal("show");
}

$('#btn_save_prov').click(function() {
  if($('#SEL_PROVINCE').val() != "") {
      $('#btn_save_prov').hide();
      $('#btn_cf_save_prov').show();
  }
  else {
...
$('#btn_cf_save_prov').click(function() {
  var url = "ajax_data.aspx?qrs=SP_UPDATE_PROVINCE_ELECTRIC_MASTER";
