        ...
        success: function( data ) { 
          if (data.length > 0) {
            ...
            $.each(data,function( i,item ) {
              var CONTRACT_ID = '<a title="แก้ไข" href="#" role="button" onclick="modalEditMaster(\'' + item.CONTRACT_ID + '\')"><span class="glyphicon icon-feather-edit3"></span></a> ' + item.CONTRACT_ID + "";

              txt_html += "" +
              "<tr>" + 
              "<td>" + NUM + "</td>" + 
              "<td>" + CONTRACT_ID + "</td>" + 
              "<td>" + itemNull(item.LOCATION) + "</td>" + 
        ...
        success: function( data ) { 
          ...
          if (data.length > 0) {
            ...
            $.each(data,function( i,item ) {
              ...
              "<td align='right'>" + parseFloat(item.VAT).toFixed(2).toLocaleString() + "</td>" + 
