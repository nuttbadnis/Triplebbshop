...
function reLoadUser(emp_id){
    var url = mainurl + 'userJson.aspx?qrs=loadUserByID&emp_id=' + emp_id + "&permission_id=" + $('#hide_this_edit').val();
        console.log(url);
        ...
        $.ajax({
            ...
            success: function( data ) { 
                if(data.length > 0){
                    var vTr = "";
                    vTr += "<tr id='" + data[0].emp_id + "' class='just-update'>"
                    ...
                    if( $('#hide_upermiss').val() == $('#hide_this_edit_by').val() ){
                        ...
                        vTr += "<button title='Edit' type='button' class='btn btn-warning btn-sm' onclick='editUser(\"" + data[0].emp_id + "\", \"" + data[0].uemail + "\", \"" + data[0].ro + "\", \"" + data[0].cluster + "\", \"" + data[0].province + "\")'><span class='glyphicon glyphicon-edit'></span></button> "
                        vTr += "<button title='Remove' type='button' class='btn btn-danger btn-sm' onclick='removeConfirm(\"" + data[0].emp_id + "\", \"" + data[0].uemail + "\")')'><span class='glyphicon glyphicon-remove'></span></button>"
                        vTr += "</td>"
                    }
                    else {
...
function reSyncJas(emp_id, uemail){
...
function editUser(id, uemail, ro, cluster, province){
    $('#sel_cluster').prop('disabled', true);
    $('#sel_province').prop('disabled', true);
...
function saveEditUPermiss() {
    var emp_id = $('#hide_id').val();
    var uemail = $('#hide_new_uemail').val();
    var ro = $('#sel_ro').val();
    var cluster = $('#sel_cluster').val();
    var province = $('#sel_province').val();

    if(emp_id.length > 0 && uemail.length > 0 && ro.length > 0 && cluster.length > 0 && province.length > 0){
        $.ajax({
            ...
            success: function( res ) {
                if(res > 0) {
                    $('#modal_edit').modal("hide");
                    reSyncJas(emp_id, uemail);
                    $('html, body').animate({scrollTop:$('#'+emp_id).position().top}, 'slow');
                }
                else {
                    modalAlert('ไม่สำเร็จ');
                }
