...
function checkRepIn(item,otct,txt = '') {
	...
	if(txt == "txt"){
		if(timeDiff(keep_time,in_serv) < 0)
			return "<span title='มีข้อมูลเข้าส่วนกลาง \"ก่อน\" เวลาบันทึกเปิด/ปิด' style='color:#ffb0b0; font-weight:bold;'>" + in_serv + "</span>";
		...
		else if(timeDiff(keep_time,in_serv) < 0)
			return "<td title='มีข้อมูลเข้าส่วนกลาง \"ก่อน\" เวลาบันทึกเปิด/ปิด' style='background-color:#ffb0b0; font-weight:bold;'>" + in_serv + " </td>";
		else if(timeDiff(keep_time,in_serv) >= 15)
