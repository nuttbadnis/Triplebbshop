...
Partial Class app_xport_target_province
    ...
    Private Sub ExportData()
        If Request.QueryString("cluster") <> Nothing Then
            ...
            Dim xStartDate As string = Request.QueryString("start_date")
            Dim xEndDate As string = Request.QueryString("end_date")

            Dim vSql As String 
            ' vSql += "EXEC dbo.SP_EXPORT_L40_TARGET_PROV_DAILY '" & xCluster & "' "

            vSql += "DECLARE @d1 date = convert(date, '" + xStartDate + "', 103) "
            vSql += "DECLARE @d2 date = convert(date, '" + xEndDate + "', 103) "

            vSql += "EXEC dbo.SP_EXPORT_L40_TARGET_PROVINCE '" & xCluster & "', @d1, @d2"

            Dim arrStartDate As String() = xStartDate.Split(New Char() {"/"c})
            Dim arrEndDate As String() = xEndDate.Split(New Char() {"/"c})
            
            Dim vSaveName As String = "MEMBERRPT_L40_TARGET_PROVINCE_[" + xCluster + "]"
