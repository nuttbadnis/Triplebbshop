...
Partial Class _json
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If qrs = "sp_report_reconcile_error" Then
            sp_report_reconcile_error()
        End If

        If qrs = "sp_report_reconcile_error_coin" Then
            sp_report_reconcile_error_coin()
        End If

        If qrs = "sp_report_reconcile_time" Then
            sp_report_reconcile_time()
        End If

        If qrs = "sp_report_reconcile_time_coin" Then
            sp_report_reconcile_time_coin()
        End If

        If qrs = "sp_update_reconcile_error" Then
            sp_update_reconcile_error()
        End If

        If qrs = "sp_update_reconcile_error_coin" Then
            sp_update_reconcile_error_coin()
        End If
        ...
        If qrs = "redisProdGetPoint" Then
            redisProdGetPoint()
        End If

        If qrs = "redisTestGetPoint" Then
            redisTestGetPoint()
        End If
    ...
    Protected Sub sp_report_reconcile_error()

        Dim vSql As String = ""
        vSql += "EXEC dbo.sp_report_reconcile_error "

        ' Response.Write(vSql)
        Response.Write(CP.rJsonDBv4(vSql, "DBRECONPOINT"))
    End Sub

    Protected Sub sp_report_reconcile_error_coin()

        Dim vSql As String = ""
        vSql += "EXEC dbo.sp_report_reconcile_error_coin "

        ' Response.Write(vSql)
        Response.Write(CP.rJsonDBv4(vSql, "DBRECONPOINT"))
    End Sub

    Protected Sub sp_report_reconcile_time()

        Dim vSql As String = ""
        vSql += "EXEC dbo.sp_report_reconcile_time "

        ' Response.Write(vSql)
        Response.Write(CP.rJsonDBv4(vSql, "DBRECONPOINT"))
    End Sub

    Protected Sub sp_report_reconcile_time_coin()

        Dim vSql As String = ""
        vSql += "EXEC dbo.sp_report_reconcile_time_coin "

        ' Response.Write(vSql)
        Response.Write(CP.rJsonDBv4(vSql, "DBRECONPOINT"))
    End Sub

    Protected Sub sp_update_reconcile_error()
        Dim error_id As String = Request.Form("error_id")
        Dim update_type As String = Request.Form("update_type")
        Dim update_by As String = Request.Form("update_by")

        Dim vSqlIn As String = ""
        vSqlIn += "EXEC dbo.sp_update_reconcile_error '" & error_id & "', '" & update_type & "', '" & update_by & "' "

        ' Response.Write(vSqlIn)
        Response.Write(DBRECONPOINT.ExecuteNonQuery(vSqlIn).ToString())
    End Sub

    Protected Sub sp_update_reconcile_error_coin()
        Dim error_id As String = Request.Form("error_id")
        Dim update_type As String = Request.Form("update_type")
        Dim update_by As String = Request.Form("update_by")

        Dim vSqlIn As String = ""
        vSqlIn += "EXEC dbo.sp_update_reconcile_error_coin '" & error_id & "', '" & update_type & "', '" & update_by & "' "
    ...
    Protected Sub redisProdGetPoint()
        Dim xAcc As String = Request.QueryString("acc")

        CM.redisProdGetPoint(xAcc)
    End Sub

    Protected Sub redisTestGetPoint()
        Dim xAcc As String = Request.QueryString("acc")

        CM.redisTestGetPoint(xAcc)
    End Sub
